...
/*   Booking Endpoints   */
export const createBooking = async (startDate, endDate, { boatId, subtotalPrice, totalPrice, duration, belongsTo, customerStripePaymentMethod, numberOfGuests }) => {
	//TODO: force startDate / endDate time zone to match the boat's location instead of the user's timezone
	const result = await fetch(baseUrl(`/bookings/create/`), POST_FETCH_OPTIONS({ boatId, subtotalPrice, totalPrice, startDate, endDate, duration, belongsTo, customerStripePaymentMethod, numberOfGuests }, true))
	const data = await result.json()
	return data
}
